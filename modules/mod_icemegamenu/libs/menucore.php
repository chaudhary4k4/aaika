...
class IceMenuTree extends JTree
{
	...
	function toXML()
	{
		...
		while($this->_current->hasChildren())
		{
			...
			$this->_buffer .= '<ul class="nav navbar-nav">';
	...
	function _getLevelXML($depth, $widthParent=0, $cols_widthParent=0, $colsParent=0, $arr_cols=array(), $parentId=0, $countChild=0, $colsWidth = 0)
	{
            ...
            {    
				...
				{   
					...
					{    
						...
						$this->_buffer .= '<div class="iceCols"><ul>'; 
					}  
				}
            }       
        }
		...
		if($iParams->get("icemega_subtitle", "") != "")
		{
			$megamenu =$iParams->get("icemega_subtitle","");
		}
		
        $ClassStyleLi = "dropdown iceMenuLiLevel_".$depth;
        
       //if($depth==1){ $ClassStyleLi = $ClassStyleLi." parent";}
	   if($this->_current->hasChildren()) {$ClassStyleLi = $ClassStyleLi.' ' .$megamenu;}
       
       $active_class = "";
       if($this->_current->id == $this->_currentItemId || $this->_current->id == JRequest::getVar('Itemid'))
            $active_class = " active";
        
		$this->_buffer .= '<li class="'.$ClassStyleLi.$active_class.'">';
		...
		while($this->_current->hasChildren())
		{
            ...
            $classStyle = "dropdown-menu";
            if($depth==1) $classStyle = "dropdown-menu";
			$classStyle .=" ".$item_class;
            $width =($width == 'auto') ? 'auto' : intval($width).'px';

			$this->_buffer .= '<ul class="'.$classStyle.'">';
            ...
            if(!(is_array($cols_width) && count($cols_width)>1))      
             ...
             if($icemega_cols_items = $this->_current->getParam("cols_items","")){
				$icemega_cols_items = str_replace(array("'",'"','<','>','<?','?>
','(',')'),"", $icemega_cols_items);
...
';
		...
		$data = '<a href="'.$tmp->url.'" class="'.$active_class.' '.$myClass.'">'.$image.$tmp->name.'</a>';
					if($item->level==1)
        {
					$data = '<a data-toggle="dropdown" href="'.$tmp->url.'" class="'.$active_class.' '.$myClass.'">'.$image.$tmp->name.'</a>';
					break;
		}
