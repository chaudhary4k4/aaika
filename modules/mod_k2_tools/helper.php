...
class modK2ToolsHelper
{
	...
	public static function treerecurse(&$params, $id = 0, $level = 0, $begin = false)
	{
		...
		{
			$output .= '<ul class="arrows_list1 level'.$level.'">';
			foreach ($rows as $row)
			{
				...
				{
					$output .= '<li'.$active.'><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'"><i class="fa fa-caret-right"></i>&nbsp;&nbsp;&nbsp;<span class="catTitle">'.$row->name.'</span><span class="catCounter">'.$row->numOfItems.'</span></a>';
					modK2ToolsHelper::treerecurse($params, $row->id, $level + 1);
					$output .= '</li>';
				}
				else
				{
					$output .= '<li'.$active.'><a href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.urlencode($row->alias)))).'"><i class="fa fa-caret-right"></i>&nbsp;&nbsp;&nbsp;<span class="catTitle">'.$row->name.'</span><span class="catCounter">'.$row->numOfItems.'</span></a></li>';
				}
			}
			$output .= '</ul>';
		}
